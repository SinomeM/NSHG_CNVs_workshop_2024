...
```{r}
## can we add clarity by looking at CNVRs in the associated locus

# subset CNVRs to those spanning the locus (also for simplicity, restrict to deletions)
...
cnvrRes=data.frame(matrix(rep("NA",nrow(tmpBins)*length(resHeader)),ncol=length(resHeader)))
...
2. overlapping or adjacent CNVRs may contribute to one association signal
   (although not clear in this case)

## What about genes?

```{r}
  ### IS THERE A WAY TO ADDRESS THE FUNCTIONAL EFFECT OF CNVs MORE DIRECTLY ?
  # One way would be to group deletions by whether they overlap the same gene
  ...
  geneRes=data.frame(matrix(rep("NA",nrow(tmpBins)*length(resHeader)),ncol=length(resHeader)))
...
Let's try to use CNVRs from the very firt scan to see if we detect some more signals.

```{r}
  ###### Should we try running the whole genome again, using CNVRs and genes instead of equally sized bins?
  ###### (Is this more likely to reflect some biologic functionality, or not ? (something to reflect on))
  ...
  ##################################################################################	
  ## NB!  The cnvrResGW will inherit the complete (del+dup) count for any CNVR,   ##
  ## even if only either del or dup is being assessed (as indicated in GT column) ##
  ##################################################################################	


  ### THERE WERE NO ADDITIONAL ASSOCIATION SIGNALS IDENTIFIED IN THIS PART
  ## DOES THAT TEACH US ANYTHING (NOT REALLY, SINCE THIS IS SIMULATED DATA)
```


## Start over, use genes directly

We can also try using genes direclty in the fist genome wide scan.
...
```{r}
  ...
  ### THERE WERE ALSO NO ADDITIONAL ASSOCIATION SIGNALS IDENTIFIED IN THIS PART
  ## DOES THAT TEACH US ANYTHING (NOT REALLY, SINCE THIS IS SIMULATED DATA)
```
